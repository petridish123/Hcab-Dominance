//  ****************************************************************************************************
// 
//      This file code defines CAB agents.  It was obtained from the supplementary material for the following paper:
// 
...
// 
//      Obtained from: https://github.com/jakecrandall/IJCAI2024_SM.git
// 
//  ****************************************************************************************************
...
public:
    ...
    GeneAgent() {
//        dirSTATE = "/home/watchtower/Datasets/JuniorHighNetwork/BehaviorStudy/State/";
...
//        exit(1);
    ...
    GeneAgent(string geneStr, int _numGeneCopies) {
        ...
        if (geneStr == "") {
            ...
            if (numGenes != (predef_NUMGENES * numGeneCopies)) {
                printf("there has been confusion about the number of genes I should have; %i vs %i\n", predef_NUMGENES, numGenes);
                exit(1);
            }
    ...
    GeneAgent(const vector<int> &genes, int _numGeneCopies) {
        numGeneCopies = _numGeneCopies;

        numGenes = genes.size();
        if (numGenes != (predef_NUMGENES * numGeneCopies)) {
            printf("wow confusion about the number of genes I should have; %i vs %i\n", predef_NUMGENES, numGenes);
            exit(1);
        }
    ...
    //// This function is only needed when we dont want the agent to remember what transactions it decided.  (example: when we are using swarm optimization to match human performance)
    ...
    virtual void playRound(int numPlayers, int numTokens, int playerIdx, int roundNum, double *received, double *popularities, double **influence, int *allocations) {
        if (roundNum != 0) {
            updateIndebtedness(roundNum-1, playerIdx, numPlayers, prevAllocations, prevPopularities);
        }

        if (!paramsHaveBeenSet) {
            throw runtime_error("The game parameters have not been set yet!");
        }
        ...
        if (roundNum > 0) {
            ...
            printT(playerIdx, "playRound");
            printT(playerIdx, "\tplayerIdx: " + to_string(playerIdx));
            printT(playerIdx, "\troundNum: " + to_string(roundNum));
            printT(playerIdx, "\tplayerIdx: " + to_string(playerIdx));
            printT(playerIdx, "\tnumPlayers: " + to_string(numPlayers));
            printT(playerIdx, "\tremainingToks: " + to_string(remainingToks));
            printT(playerIdx, "\tattackAlloc: " + to_string(*attackAlloc));
    ...
    // ******************************************************************
//
//      Now all the functions and data members that make it happen
//
// ******************************************************************
protected:
    int theTracked;
    ...
    virtual void setUpRand() {
        randCount = 0;
        ifstream input(dirSTATE + "rnums.txt");
    ...
    virtual vector<string> parse(const string& s) {
        vector<string> tokens;
        string token;
        istringstream tokenStream(s);
    ...
    virtual void assassinGenes() {
        ...
        genes[GENE_vengeanceMultiplier] = 100;
        genes[GENE_vengeanceMax] = 100;
        genes[GENE_vengeancePriority] = 100;
    ...
    virtual void deleteAll(int nmPlayers) {
        if (isInitialized) {
    ...
    virtual void initVars(int playerIdx, int numPlayers, double *popularities) {
        popHistory.clear();
    ...
    virtual void updateVars(double *received, double *popularities, int numTokens, int numPlayers, int playerIdx) {
        int currentPool = determineGenePool(playerIdx, numPlayers, popularities);
        int empiezaGene = (currentPool * (numGenes / numGeneCopies));
    ...
    virtual int determineGenePool(int playerIdx, int numPlayers, double *popularities) {
    ...
    virtual void findContract(int index) {
    ...
    virtual void updateIndebtedness(int roundNum, int playerIdx, int numPlayers, int *allocations, double *popularities) {
        for (int i = 0; i < numPlayers; i++) {
            if (i == playerIdx)
                tally[i] = 0.0;
            else
    ...
    virtual void printT(int ind, string msg) {
        if (ind == theTracked)
            cout << msg << endl;
    }

    virtual string vec2String(double *vec, int len) {
        string s;
        for (int i = 0; i < len; i++) {
            s += to_string(vec[i]) + " ";
        }
    ...
    virtual string vec2String(int *vec, int len) {
        string s;
        for (int i = 0; i < len; i++)
            s += to_string(vec[i]) + " ";
    ...
    virtual string vec2String(bool *vec, int len) {
        string s;
        for (int i = 0; i < len; i++)
    ...
    virtual double getMax(double o1, double o2) {
        if (o1 > o2)
            return o1;
        return o2;
    }

    virtual void getScaleBack(int playerIdx, int numPlayers) {
    ...
    virtual void computeUsefulQuantities(int roundNum, int numPlayers, int playerIdx, int numTokens, double **influence) {
        int i, j;
    ...
    virtual double sumMCol(double **x, int ind, int numPlayers) {
        double s = 0.0;
        for (int i = 0; i < numPlayers; i++)
            s += x[i][ind];
    ...
    virtual double sumMRow(double **x, int ind, int numPlayers) {
        double s = 0.0;
        for (int i = 0; i < numPlayers; i++)
            s += x[ind][i];
    ...
    virtual double sumVec(double *v, int len) {
        double sm = 0.0;
        for (int i = 0; i < len; i++)
            sm += v[i];
    ...
    virtual double sumVec(int *v, int len) {
        int sm = 0.0;
        for (int i = 0; i < len; i++)
            sm += v[i];
    ...
    virtual string getCommunitiesString(vector< set<int> > c) {
        string str = "{";
    ...
    virtual CommunityEvaluation *groupAnalysis(int roundNum, int numPlayers, int playerIdx, double *popularities, double **influence, vector< set<int> > &communities) {
    ...
    virtual CommunityEvaluation *envisionCommunities(int numPlayers, int playerIdx, double *popularities, double **influence, double **A_pos, double **A_neg, vector< set<int> > communities_ph1, vector< set<int> > communities, double modularity) {
    ...
    virtual bool alreadyIn(set<int> s, vector<CommunityEvaluation *> potentialCommunities) {
        for (int c = 0; c < potentialCommunities.size(); c++) {
    ...
    virtual string getSetString(set<int> s) {
        string str = "";
    ...
    virtual void removeMostlyDead(set<int> *s, int playerIdx, double *popularities) {
    ...
    virtual double determineCommunities(int numPlayers, int playerIdx, double *popularities, vector< set<int> > c, int s_idx, double **A_pos, double **A_neg) {
        // cout << "x: " << s_idx << endl;
    ...
    virtual double computeSignedModularity(int numPlayers, int *cur_comm, double **A_pos, double **A_neg) {
        auto *k = new double[numPlayers];
        for (int i = 0; i < numPlayers; i++) {
    ...
    virtual int findCommunity(int playerIdx, vector< set<int> > communities) {
        for (int i = 0; i < communities.size(); i++) {
    ...
    virtual void enumerateCommunities(double modularity_ph1, vector< set<int> > communities_ph1, double modularity, vector< set<int> > communities_mega, vector< set<int> > &communities) {
    ...
    virtual CommunityEvaluation *randomSelections(int numPlayers, int playerIdx, double *popularities) {
        // set<int> plyrs;
        // for (int i = 0; i < numPlayers; i++) {
        //     if (playerIdx != i)
        //         plyrs.insert(i);
    ...
    virtual double computeCoalitionTarget(int roundNum, int numPlayers, double *popularities, vector< set<int> > communities, int playerIdx) {
    ...
    virtual void computeAdjacencyMatrices(int numPlayers) {
    ...
    virtual double louvainMethod_phase1(int numPlayers, double **A_pos, double **A_neg, vector< set<int> > &communities) {
        if (numPlayers == 0)
            return 0.0;
    ...
    virtual double louvainMethod_phase2(int numPlayers, double **A_pos, double **A_neg, double modularity_ph1, vector< set<int> > communities_ph1, vector< set<int> > &communities_mega) {
    ...
    virtual double computeModularity(int numPlayers, int *currentCommunities, double **A, double *k, double m) {
        if (m == 0)
            return 0.0;
    ...
    virtual int deltar(int *currentCommunities, int i, int j) {
    ...
    virtual double computeModularity2(int numPlayers, vector< set<int> > communities, double **A) {
    ...
    virtual int deltar2(vector< set<int> > c, int i, int j) {
        for (int s = 0; s < c.size(); s++) {
    ...
    virtual double move_i_to_j(int numPlayers, int **comMatrix, double m, double *K, double **A, int i, int com_j, int com_i) {
    ...
    virtual double vecDotProduct(double *v1, double *v2, int numPlayers) {
        double s = 0.0;
        for (int i = 0; i < numPlayers; i++) {
            s += v1[i] * v2[i];
        }
    ...
    virtual double vecDotProduct(int *v1, double *v2, int numPlayers) {
        double s = 0.0;
        for (int i = 0; i < numPlayers; i++) {
            s += v1[i] * v2[i];
        }
    ...
    virtual double vecColDotProduct(double *v, double **M, int index, int numPlayers) {
        double s = 0.0;
        for (int i = 0; i < numPlayers; i++) {
            s += v[i] * M[i][index];
        }
    ...
    virtual double vecColDotProduct(int *v, double **M, int index, int numPlayers) {
        double s = 0.0;
        for (int i = 0; i < numPlayers; i++) {
            s += v[i] * M[i][index];
        }
    ...
    virtual double getCentrality(set<int> s, int playerIdx, double *popularities) {
    ...
    virtual double getCollectiveStrength(double *popularities, set<int> s, double curCommSize, int numPlayers) {
        double proposed = 0.0;
        for (set<int>::iterator i = s.begin(); i != s.end(); i++) {
    ...
    virtual double getFamiliarity(set<int> s, int playerIdx, int numPlayers, double **influence) {
    ...
    virtual double getIngroupAntisocial(set<int> s, int playerIdx) {
        double scl = 1.0;
    ...
    virtual int groupGivings(int roundNum, int numPlayers, int numTokens, int numGivingToks, int playerIdx, double **influence, double *popularities, CommunityEvaluation *selectedCommunity, int *attackAlloc, int *groupAlloc) {
        if (numGivingToks <= 0) {
            for (int i = 0; i < numPlayers; i++)
                groupAlloc[i] = 0;
            return 0;
    ...
    virtual int groupAllocateTokens(int playerIdx, int numPlayers, int numGivingTokens, int roundNum, double **influence, double *popularities, CommunityEvaluation *selectedCommunity, int *attackAlloc, int *groupAlloc) {
    ...
    virtual int dialBack(int numPlayers, int numTokens, int playerIdx, int *groupAlloc, double *popularities) {
    ...
    virtual void estimateKeeping(int playerIdx, int numPlayers, int numTokens, vector< set<int> > communities) {
        for (int i = 0; i < numPlayers; i++) {
    ...
    virtual double fearKeeping(int numPlayers, int playerIdx, vector< set<int> > communities, int agentIdx) {
    ...
    virtual int cuantoGuardo(int roundNum, int playerIdx, int numPlayers, int numTokens, double *popularities, double *received, set<int> selectedCommunity) {
        // Change on July 12
    ...
    virtual int quienAtaco(int roundNum, int playerIdx, int numPlayers, int numTokens, int remainingToks, double *popularities, double **influence, set<int> selectedCommunity, vector< set<int> > communities, int *attackToks) {
        ...
        attackObject defendFriendChoice = defendFriend(playerIdx, roundNum, numPlayers, numTokens, remainingToks, popularities, influence, selectedCommunity, communities, groupCat);
        ...
        if (vengenceChoice.index >= 0) {
            vengenceChoice.priority = genes[GENE_vengeancePriority];
    ...
    virtual attackObject defendFriend(int playerIdx, int roundNum, int numPlayers, int numTokens, int remainingToks, double *popularities, double **influence, set<int> selectedCommunity, vector< set<int> > communities, int *groupCat) {
        printT(playerIdx, "defendFriend: ");
        printT(playerIdx, "\tplayerIdx: " + to_string(playerIdx));
        printT(playerIdx,"\tnumPlayers: " + to_string(numPlayers));
        printT(playerIdx,"\tnumTokens: " + to_string(numTokens));
        printT(playerIdx,"\tremainingToks: " + to_string(remainingToks));
        printT(playerIdx,"\tpopularities: " + str(popularities, numPlayers));
        printT(playerIdx,"\tinfluence: \n" + str(influence, numPlayers, numPlayers));
        printT(playerIdx,"\tselectedCommunity: " + str(selectedCommunity, numPlayers));
        printT(playerIdx,"\tgroupCat: " + to_string(*groupCat));
        ...
        printT(playerIdx, "myCommVec: " + str(myCommVec, numPlayers));
        ...
        printT(playerIdx, "\tmyCommVec: " + str(myCommVec, numPlayers));
        printT(playerIdx, "\tbadMarks: " + str(badMarks, numPlayers));
        ...
        if (worstInd >= 0) {
            ...
            printT(playerIdx, "\tpopularities[playerIdx]: " + to_string(popularities[playerIdx]));
            printT(playerIdx, "\tattackStrength: " + to_string(attackStrength));
            ...
            printT(playerIdx, "\t-(int)(myPart+0.5): " + to_string(-(int)(myPart+0.5)));
            printT(playerIdx, "\t(genes[GENE_defendFriendMax] / 100.0): " + to_string((genes[GENE_defendFriendMax] / 100.0)));
            printT(playerIdx, "\t-(int)(((genes[GENE_defendFriendMax] / 100.0) * numTokens) + 0.5): " + to_string((((genes[GENE_defendFriendMax] / 100.0) * numTokens) + 0.5)));
            printT(playerIdx, "\t-getMax(-(int)(myPart+0.5), -(int)(((genes[GENE_defendFriendMax] / 100.0) * numTokens) + 0.5)): " + to_string(-getMax(-(int)(myPart+0.5), -(int)(((genes[GENE_defendFriendMax] / 100.0) * numTokens) + 0.5))));
            ...
            if ((cantidad >= (myPart - 1)) && (tokensNeeded > 0)) {
                ...
                if (vengenceAdvantage > 0.0) {
                    ...
                    printT(playerIdx, "\tworstInd: " + to_string(worstInd));
                    printT(playerIdx, "\tcantidad: " + to_string(cantidad));
                    printT(playerIdx, "\tvengenceAdvantage: " + to_string(vengenceAdvantage));
    ...
    virtual attackObject takeVengence(int roundNum, int playerIdx, int numPlayers, set<int> selectedCommunity, int numTokens, int remainingToks, double *popularities, double **influence) {
        attackObject vengenceDefault(-1, 0, 0.0);

        if ((popularities[playerIdx] <= 0.0) || (genes[GENE_vengeancePriority] < 50))
            return vengenceDefault;

        double multiplicador = genes[GENE_vengeanceMultiplier] / 33.0;
        double vengenceMax = numTokens * (genes[GENE_vengeanceMax] / 100.0);
    ...
    virtual void groupCompare(int numPlayers, int playerIdx, double *popularities, vector< set<int> > communities, int *groupCat) {
        int i;
    ...
    virtual attackObject pillageTheVillage(int roundNum, int playerIdx, int numPlayers, set<int> selectedCommunity, int numTokens, int remainingToks, double *popularities, double **influence, int *groupCat) {
    ...
    virtual attackObject selectAttack(vector<attackObject> attackPossibilities) {
        double mag = 0.0;
